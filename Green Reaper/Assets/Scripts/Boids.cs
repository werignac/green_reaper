...
public class Boids : MonoBehaviour
{
    ...
    [SerializeField]
    private GameObject CrowWeapon;
    private GameObject currentCrowWeapon;
    public int numBoids;
    ...
    private void CheckForDropOffAndEnd(IndividualBoid boid)
    {
        ...
        if (distanceToEndPoint < closeEnoughToSteal)
        {
            ...
            weaponPickupInstance.SetSprites(currentCrowWeapon.GetComponentInChildren<ScytheSpriteManager>());
    ...
    private void StealAndScatter(IndividualBoid leadBoid)
    {
        ...
        currentCrowWeapon = Instantiate(CrowWeapon, leadBoid.obj.transform.position, Quaternion.identity);
        Rigidbody2D crowRigidbody = leadBoid.obj.GetComponent<Rigidbody2D>();
        crowRigidbody.simulated = true;
        currentCrowWeapon.GetComponent<HingeJoint2D>().connectedBody = crowRigidbody;
        currentCrowWeapon.GetComponentInChildren<ScytheSpriteManager>().SetFromSpriteManager(HarvestState.instance.currentWeapon.GetComponentInChildren<ScytheSpriteManager>());
